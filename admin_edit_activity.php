          ...
          <form class="col s7" method="POST" action="<?php $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
           <table class="striped responsive-table">
                ...
                <tbody>
                 ...
                 <td>
                     <input name="activity_name" type="text"  value="<?php echo $activity_name; ?>"  id="activity_name" style='text-transform:capitalize'; class="validate" data-length="10" pattern="[A-Za-z ]+" required>
                     <label for="activity_name" data-error="wrong" data-success="right"></label>
                  </td>
                  <td>
                     <input name="role" type="text" id="role"  value="<?php echo $role; ?>"  class="validate" data-length="10" style='text-transform:capitalize'; pattern="[a-zA-Z]+" required>
                     <label for="role" data-error="wrong" data-success="right"></label>
                  </td>
                  <td>
                              
                           <input name="type_activity" value='<?php echo $type_activity;?>' type="text" id="type_activity" class="validate" style='text-transform:capitalize'; data-length="30" pattern="[a-zA_Z- ]+" required>
               ...
               if(isset($_POST['submit1']))
                 {
                      ...
                      {
                            $sql = "update activity SET activity_name='$activity_name',role='$role',type_activity = '$type_activity',filename='$a' WHERE atid='$atid'";
                            $res = mysqli_query($conn,$sql);
                            if($res===true)
                            {
                      ...
                      else
                      {
                             ...
                             $sql = "update activity SET activity_name='$activity_name',role='$role',type_activity = '$type_activity' WHERE atid='$atid'";
                            $res = mysqli_query($conn,$sql);
                            if($res===true)
                            {
